...
/**
 * Mock AI Provider
 * Returns pre-defined responses for testing without API calls
 */
export class MockProvider implements AIClient {
  ...
  async stream(config: StreamConfig): Promise<string> {
    ...
    const domainIndex = Math.floor(conversationLength / 4)
    ...
    const response = this.getMockResponse(currentDomain)
  ...
  private getMockResponse(domain: DomainId): string {
...
Feature branch → Preview → Tests → Production`
    ...
    return mockResponses[domain] || mockResponses.planning
  }
}
